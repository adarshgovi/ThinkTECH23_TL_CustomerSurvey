    ...
    while(fgets(buffer, sizeof(buffer), filein)!=NULL)
    {
    	...
    	if(buffer[0]==':')
    	{
			...
			for(j=0; j<linesize; j++)
			{
				...
				chksum+=value;
				if(((address&0x0fff)+j)<MEMSIZE) FlashBuff[(address&0x0fff)+j]=value;
			}
...
int main(int argc, char **argv)
{
	...
	printf("Converting '%s' to '%s'...", HexName, MifName);
	...
	if(MaxAddress<0)
	{
		printf("\nERROR: Could not load hex file.");
		return -1;
	}
	
	fprintf(fileout, "DEPTH = 4096;\n");
